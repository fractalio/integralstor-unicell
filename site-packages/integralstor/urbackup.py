...
def get_logs(date_on=None, date_from=None, date_till=None):
    ...
    try:
        ...
        client_id = db.get_multiple_rows(urb_db, qry)

        for i in client_id[0]:
                client_id_list.append(i['id'])

        for j in client_id_list:
                qry = 'select * from logs where clientid = %d order by id desc limit 1' % (j)
                k = db.get_multiple_rows(urb_db, qry)
                if not k[0]:
                        continue
                logs.append(k[0][0])
...
def compile_report(date_on=None, date_from=None, date_till=None):
    ...
    try:
        ...
        for m in client_details[0]:
                n = filesize.bytes_to_human_readable_size(m['bytes_used_files'])
                client_space_used.update({m['id']: n[0] })
        ...
        if logs:
            for idx, log in enumerate(logs, start=1):
                ...
                if log['clientid'] in client_space_used:
                    d['bytes_used'] = client_space_used[log['clientid']]  
...
def generate_pdf_report(name=None, date_on=None, date_from=None, date_till=None):
    try:
        ...
        elements.append(Paragraph('<para align=left spaceb=3>%s</para>' % description, styles['Normal']))
        ...
        tmp = sorted(data[1:], key=lambda x: datetime.datetime.strptime(x[3], '%Y-%m-%d %H:%M:%S'))
        tmp.insert(0, data[0])
        for si_no, i in enumerate(tmp[1:], start=1):
            i[0] = si_no
